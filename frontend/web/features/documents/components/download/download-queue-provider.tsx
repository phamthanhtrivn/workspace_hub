...
import { Download, X, CheckCircle2, AlertCircle, FolderArchive } from "lucide-react";
...
interface DownloadQueueContextValue {
  enqueueDownload: (documentId: string, folderName: string) => void;
}
...
export function DownloadQueueProvider({ children }: { children: React.ReactNode }) {
  ...
  const enqueueDownload = useCallback(
    (documentId: string, folderName: string) => {
      ...
      documentsApi
        .downloadFolderAsZip(documentId, folderName, (percent) => {
          updateTask(taskId, { progress: percent, status: "downloading" });
        })
        .then(() => {
  ...
  return (
    <DownloadQueueContext.Provider value={{ enqueueDownload }}>
      ...
      <div className="fixed bottom-4 right-4 z-50 w-80 rounded-2xl bg-slate-800 shadow-2xl shadow-black/30 border border-slate-700/50 overflow-hidden animate-in slide-in-from-bottom-4 duration-200">
        {/* Header */}
        <div
          className="flex items-center justify-between px-4 py-3 cursor-pointer hover:bg-slate-700/40 transition-colors"
          ...
          <div className="flex items-center gap-1">
            {tasks.some((t) => t.status === "done" || t.status === "error") && (
              <button
                ...
                className="text-[10px] font-bold text-slate-400 hover:text-slate-200 px-2 py-0.5 rounded hover:bg-slate-700 transition-colors"
              >
                Xóa tất cả
              </button>
            )}
            <button
              ...
              className="w-6 h-6 flex items-center justify-center rounded-lg hover:bg-slate-700 text-slate-400 hover:text-slate-200 transition-colors text-xs font-bold"
            >
              {isCollapsed ? "▲" : "▼"}
            </button>
          </div>
        ...
        {!isCollapsed && (
          <div className="max-h-56 overflow-y-auto divide-y divide-slate-700/40">
            {tasks.map((task) => (
              <DownloadTaskRow key={task.id} task={task} onRemove={removeTask} />
            ))}
          </div>
        )}
      </div>
...
function DownloadTaskRow({
  ...
  return (
    <div className="px-4 py-3 flex flex-col gap-1.5">
      ...
      {task.status === "error" && (
        <p className="text-[10px] text-red-400 font-medium truncate">{task.error}</p>
      )}
    </div>
  );
}
