...
import { getPendingInvitations, acceptInvitation, declineInvitation, getPublicProfile } from "../api/chat.api";
...
export default function InvitationList({ currentUserId, onAccept }: InvitationListProps) {
    ...
    const fetchInvitations = async () => {
      try {
        ...
        if (response?.success) {
          ...
          const uniqueInviters = Array.from(new Set(response.data.map((inv: any) => inv.invitedBy)));
          const profiles: Record<string, any> = {};
          await Promise.all(uniqueInviters.map(async (id: any) => {
      ...
      {isOpen && (
        <div className="absolute top-12 left-0 w-80 bg-white shadow-xl rounded-xl border border-gray-100 z-50 p-2 overflow-hidden animate-in fade-in slide-in-from-top-2">
          <div className="px-3 py-2 border-b border-gray-100 flex justify-between items-center mb-2">
            <h3 className="font-bold text-sm text-gray-800">Lời mời nhóm ({invitations.length})</h3>
          ...
          <div className="max-h-80 overflow-y-auto space-y-2">
            {invitations.length === 0 ? (
              <p className="text-sm text-center text-gray-500 py-6">Không có lời mời nào</p>
            ) : (
              invitations.map(inv => {
                    ...
                    <div className="flex items-center gap-3 mb-3">
                      <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center overflow-hidden flex-shrink-0">
                        {inv.conversation?.avatarUrl ? (
                          <img src={inv.conversation.avatarUrl} alt="Group" className="w-full h-full object-cover" />
                        ) : (
                          <span className="font-bold text-blue-500">{inv.conversation?.name?.charAt(0) || "G"}</span>
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-gray-800 truncate">{inv.conversation?.name || "Nhóm trò chuyện"}</p>
                        <p className="text-xs text-gray-500 truncate">Mời bởi {inviter?.fullName || "Ai đó"}</p>
                      </div>
                    </div>
