...
import { DocumentItemType } from "../../../documents/types/documents.enums";
import { DocumentItem } from "../../../documents/types/documents.types";
...
export default function MyFilesSelectModal({
  ...
  const { data: response, isLoading } = useQuery({
    ...
    queryFn: () => documentsApi.getDocuments({ folderId: currentFolderId || undefined }),
    enabled: isOpen,
  });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200 flex flex-col max-h-[500px]">
        ...
        <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100">
          <h2 className="text-lg font-black text-gray-800">Select from My Files</h2>
          <button
            ...
            <React.Fragment key={history.id || "root"}>
              ...
              <button
                ...
                className={`cursor-pointer text-xs font-bold whitespace-nowrap transition-colors hover:text-blue-600 ${
                  idx === folderHistory.length - 1 ? "text-gray-800 font-extrabold" : "text-gray-400"
                }`}
              >
                {history.name}
              </button>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <Folder size={18} className="text-blue-500 fill-blue-50 shrink-0" />
