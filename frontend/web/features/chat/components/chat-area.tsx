...
import JumpToRecentBanner, {
  ScrollToBottomButton,
} from "./jump-to-recent-banner";
...
export default function ChatArea({
  ...
  const handleSendMessageWithMedia = useCallback(
    async (
      content: string,
      medias?: import("../types/chat-socket.types").SendSocketMessageMedia[],
      mentions?: string[],
    ) => {
      await handleSendMessageAction(
        content,
  ...
  return (
    <div className="flex-1 flex flex-col bg-white h-full min-h-0 relative">
      ...
      {hasPreviousPage && (
        <JumpToRecentBanner onJumpToRecent={handleJumpToRecent} />
      )}
