...
function StatusCircles({
  ...
  return (
    <div className="flex items-center gap-1 text-[10px] font-bold ml-3">
      <span
        className="inline-flex items-center justify-center min-w-5 h-5 rounded-full bg-[#DFE1E6] text-[#42526E] px-1.5"
        title="To Do"
      >
        {counts.todo}
      </span>
      <span
        className="inline-flex items-center justify-center min-w-5 h-5 rounded-full bg-[#DEEBFF] text-[#0747A6] px-1.5"
        title="In Progress"
      >
        {counts.progress}
      </span>
      <span
        className="inline-flex items-center justify-center min-w-5 h-5 rounded-full bg-[#E3FCEF] text-[#006644] px-1.5"
        title="Done"
      >
        {counts.done}
      </span>
...
export default function ListView({
  ...
  const [activeInlineCreatorId, setActiveInlineCreatorId] = useState<
    string | null
  >(null);
      ...
      >
        ...
        <div className="shrink-0 w-8 flex justify-end">
          {task.assignees.length > 0 ? (
            ...
            <span className="h-5 w-5 rounded-full border border-dashed border-slate-300 bg-slate-50 flex items-center justify-center text-[10px] text-slate-400 font-bold">
              ?
            </span>
          )}
        </div>
    ...
    <DndContext onDragEnd={handleDragEnd}>
      ...
      {!isGeneralProject &&
        groupTasks.map((group) => {
          ...
          const sprintDateRange = formatSprintDateRange(
            group.startDate,
            group.dueDate,
          );
          ...
          return (
            <div
              key={group.id}
              className="rounded border border-slate-200 bg-[#FAFBFC] shadow-sm overflow-hidden"
            >
              {/* ── Group Header ── */}
              <div
              ...
              >
                <button
                  type="button"
                  className="grid h-5 w-5 place-items-center text-slate-600 rounded"
                >
                  {isCollapsed ? (
                ...
                <span className="text-xs text-slate-500 font-medium">
                  ({children.length}{" "}
                  {isGeneralProject ? "subtasks" : "work items"})
                </span>
              ...
              {isReordering && (
                <div className="border-b border-blue-100 bg-blue-50 px-4 py-1.5 text-[11px] font-medium text-blue-700">
                  Kéo thả các work item để sắp xếp lại, sau đó thả lên vị trí
                  mong muốn.
                </div>
              )}
      ...
      {isGeneralProject &&
        activeInlineCreatorId === "__backlog__" &&
        renderInlineCreator()}
