...
export default function TaskChatDialog({
  ...
  return (
    ...
    >
      <div
        className="flex h-[min(620px,85vh)] w-full max-w-lg flex-col rounded-xl bg-white shadow-2xl"
          ...
          {messages.length === 0 ? (
            ...
            <div className="space-y-3">
              {messages.map((item) => (
                <div
                  key={item.id}
                  className="ml-auto max-w-[82%] rounded-lg bg-blue-600 px-3 py-2 text-white shadow-sm"
                >
                  <p className="text-xs font-medium">{item.content}</p>
                  <p className="mt-1 text-right text-[9px] font-semibold text-blue-100">
                    {item.time}
                  </p>
                </div>
              ))}
