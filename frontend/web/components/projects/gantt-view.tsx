...
export default function GanttView({
  ...
  const range = (() => {
    ...
    const start = addDays(new Date(Math.min(...starts.map((date) => date.getTime()))), -2);
    const end = addDays(new Date(Math.max(...ends.map((date) => date.getTime()))), 2);
    return { start, end };
  })();
  ...
  const todayOffset = dayDifference(range.start, toLocalDate(dateKey(new Date())));
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs font-semibold text-slate-400">
              {formatRange(range.start, range.end)} · {datedTasks.length} task có lịch
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3 text-[11px] font-semibold text-slate-500">
          <span className="inline-flex items-center gap-1"><i className="h-2.5 w-2.5 rounded-full bg-slate-400" />To Do</span>
          <span className="inline-flex items-center gap-1"><i className="h-2.5 w-2.5 rounded-full bg-blue-600" />In Progress</span>
          <span className="inline-flex items-center gap-1"><i className="h-2.5 w-2.5 rounded-full bg-emerald-500" />Done</span>
        </div>
      </div>
      ...
      <div className="overflow-x-auto rounded border border-slate-200 bg-white shadow-sm">
        <div className="min-w-max">
          ...
          >
            ...
            <div className="relative grid" style={{ gridTemplateColumns: `repeat(${days.length}, ${DAY_WIDTH}px)` }}>
              {days.map((day) => (
                <div key={dateKey(day)} className="border-r border-slate-200 px-1 py-2 text-center last:border-r-0">
                  <div className="text-[9px] font-bold uppercase text-slate-400">
                    {day.toLocaleDateString("vi-VN", { weekday: "short" })}
                  </div>
                  <div className="text-xs font-black text-[#172B4D]">{formatDay(day)}</div>
                  {day.getDate() === 1 && <div className="text-[9px] font-semibold text-slate-400">{formatMonth(day)}</div>}
                </div>
              ))}
            </div>
          </div>

          {datedTasks.length > 0 ? datedTasks.map((task) => {
            const start = taskStart(task) as Date;
            const end = taskEnd(task) as Date;
            const left = Math.max(0, dayDifference(range.start, start)) * DAY_WIDTH + 4;
            const width = Math.max(32, (dayDifference(start, end) + 1) * DAY_WIDTH - 8);
            ...
            return (
              ...
              >
                ...
                <span
                  ...
                  style={{
                    backgroundImage: "linear-gradient(to right, rgb(226 232 240 / 0.7) 1px, transparent 1px)",
                    backgroundSize: `${DAY_WIDTH}px 100%`,
                  }}
                >
                  {todayOffset >= 0 && todayOffset < days.length && (
                    <span className="absolute bottom-0 top-0 w-px bg-red-300/70" style={{ left: `${todayOffset * DAY_WIDTH + DAY_WIDTH / 2}px` }} />
                  )}
                  <span
      ...
      {unscheduledTasks.length > 0 && (
        <div className="rounded border border-dashed border-slate-300 bg-slate-50 px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-black text-slate-600">Chưa lên lịch ({unscheduledTasks.length})</span>
          </div>
