...
import { Check, CirclePlay, Pencil, Plus, Rocket, RotateCcw, X } from "lucide-react";
import { SprintStatus, TaskStatus, type Sprint, type Task } from "@/types/project";
import { TaskStatusBadge } from "./status-badge";
import TaskChatButton from "./task-chat-button";
import ProjectFilePanel, { createProjectFileItems, FilePickerButton, type ProjectFileItem } from "./project-file-panel";
...
export default function SoftwareBacklogView({
  ...
  const addFiles = (files: File[], scopeLabel: string) => {
    setProjectFiles((current) => [...current, ...createProjectFileItems(files, scopeLabel)]);
  };
  ...
  const backlogTasks = activeTasks.filter((task) => !task.sprintId && !task.parentTaskId);
  ...
  const handleInlineSprintTask = async (event: React.FormEvent, sprintId: string) => {
    event.preventDefault();
  ...
  const handleDragStart = (event: React.DragEvent, taskId: string, sprintId?: string) => {
    event.dataTransfer.effectAllowed = "move";
    event.dataTransfer.setData("application/x-project-task", JSON.stringify({ taskId, sprintId }));
  };

  const readDragPayload = (event: React.DragEvent): { taskId: string; sprintId?: string } | null => {
    try {
      const payload = JSON.parse(event.dataTransfer.getData("application/x-project-task"));
  ...
  return (
    <div className="space-y-5">
      ...
      <section
        ...
        onDragOver={(event) => { event.preventDefault(); setDragOverTarget("backlog"); }}
      ...
      >
        ...
        {selectedTaskIds.length > 0 && (
          <div className="flex flex-wrap items-center gap-2 border-b border-blue-100 bg-blue-50 px-4 py-3">
            <span className="text-xs font-bold text-blue-800">{selectedTaskIds.length} task đã chọn</span>
            <select
            ...
            return (
              <div key={task.id}>
                <div className="flex items-center gap-3 px-4 py-3 hover:bg-slate-50">
                  ...
                  >
                    <span className="block truncate text-xs font-bold text-[#172B4D]">{task.title}</span>
                    <span className="mt-0.5 block text-[10px] font-semibold text-slate-400">{task.id.slice(0, 8).toUpperCase()}</span>
                  </button>
                  <TaskChatButton task={task} onOpenChat={onOpenChat} compact />
                  <TaskStatusBadge status={task.status} compact />
                </div>
                {children.filter((child) => !child.sprintId).map((child) => (
                  <div
                    key={child.id}
                    role="button"
                    tabIndex={0}
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1 truncate text-xs font-semibold text-slate-600">{child.title}</span>
                    <TaskChatButton task={child} onOpenChat={onOpenChat} compact />
        ...
        {sprints.map((sprint) => {
          ...
          const renderTaskRow = (task: Task, nested = false) => (
            <div
              key={task.id}
              draggable
              onDragStart={(event) => handleDragStart(event, task.id, sprint.id)}
            ...
            >
              ...
              <button type="button" onClick={() => onTaskClick?.(task)} className="min-w-0 flex-1 truncate text-left text-xs font-semibold text-slate-700">{task.title}</button>
              <TaskChatButton task={task} onOpenChat={onOpenChat} compact />
              <TaskStatusBadge status={task.status} compact />
            </div>
          );
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-4 py-3">
              <div className="flex items-center gap-2">
                {sprint.status === SprintStatus.ACTIVE ? <CirclePlay className="h-4 w-4 text-blue-600" /> : <Rocket className="h-4 w-4 text-slate-400" />}
                <div>
                  <h3 className="text-sm font-black text-[#172B4D]">{sprint.name}</h3>
                  <p className="text-[11px] font-semibold text-slate-400">
                    {sprint.startDate?.slice(0, 10)} → {sprint.endDate?.slice(0, 10)} · {sprint.tasks.length} task
                  </p>
                </div>
                <span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-black text-slate-600">{sprint.status}</span>
              </div>
              <div className="flex items-center gap-2">
                <FilePickerButton compact label="Add file" onFiles={(files) => addFiles(files, `Sprint: ${sprint.name}`)} />
                <button type="button" disabled={isBusy} onClick={() => openEditSprint(sprint)} className="inline-flex items-center gap-1 rounded border border-slate-200 px-2.5 py-1.5 text-[11px] font-bold text-slate-600 hover:bg-slate-50 disabled:opacity-50">
                  <Pencil className="h-3.5 w-3.5" /> Chỉnh sửa
                </button>
                {sprint.status === SprintStatus.PLANNED && (
                  <button type="button" disabled={isBusy} onClick={() => void onStartSprint(sprint.id)} className="inline-flex items-center gap-1 rounded bg-blue-600 px-2.5 py-1.5 text-[11px] font-bold text-white disabled:opacity-50">
                    <CirclePlay className="h-3.5 w-3.5" /> Start sprint
                  </button>
                )}
                {sprint.status === SprintStatus.ACTIVE && (
                  <button type="button" disabled={isBusy} onClick={() => void onCompleteSprint(sprint.id)} className="inline-flex items-center gap-1 rounded bg-emerald-600 px-2.5 py-1.5 text-[11px] font-bold text-white disabled:opacity-50">
                    <Check className="h-3.5 w-3.5" /> Complete sprint
                  </button>
                )}
                {sprint.status === SprintStatus.COMPLETED && (
                  <button type="button" disabled={isBusy} onClick={() => void onReopenSprint(sprint.id)} className="inline-flex items-center gap-1 rounded border border-blue-200 px-2.5 py-1.5 text-[11px] font-bold text-blue-700 hover:bg-blue-50 disabled:opacity-50">
                    <RotateCcw className="h-3.5 w-3.5" /> Reopen sprint
                  </button>
                )}
              </div>
            </div>
            {sprint.goal && <p className="border-b border-slate-100 px-4 py-2 text-xs font-semibold text-slate-500">Goal: {sprint.goal}</p>}
            <div className="divide-y divide-slate-100">
              {sprint.tasks.length > 0 ? (rootTasks.length > 0 ? rootTasks : sprint.tasks).flatMap((task) => [
                renderTaskRow(task),
                ...(childrenByParent.get(task.id) || []).map((child) => renderTaskRow(child, true)),
              ]) : <div className="px-4 py-6 text-center text-xs font-semibold text-slate-400">Chưa có task trong Sprint.</div>}
            </div>
            <div className="border-t border-slate-100 px-4 py-2.5">
              {inlineSprintId === sprint.id ? (
                <form onSubmit={(event) => void handleInlineSprintTask(event, sprint.id)} className="flex items-center gap-2">
                  <input
                    autoFocus
                  ...
                  />
                  <button type="submit" disabled={isBusy || !inlineTaskTitle.trim()} className="rounded bg-blue-600 px-3 py-2 text-xs font-bold text-white disabled:opacity-50">Tạo</button>
                  <button type="button" onClick={() => { setInlineSprintId(null); setInlineTaskTitle(""); }} className="rounded px-2 py-2 text-xs font-bold text-slate-500 hover:bg-slate-100">Hủy</button>
                </form>
              ) : (
                <button
                  type="button"
                  onClick={() => { setInlineSprintId(sprint.id); setInlineTaskTitle(""); }}
      ...
      {showCreateSprint && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/45 p-4">
          <form onSubmit={(event) => void handleCreateSprint(event)} className="w-full max-w-lg rounded-xl bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between">
              <div>
                <h2 className="text-lg font-black text-[#172B4D]">{editingSprint ? "Chỉnh sửa Sprint" : "Create sprint"}</h2>
                <p className="mt-1 text-xs font-semibold text-slate-400">Tạo Sprint trước, sau đó đưa task từ Backlog vào.</p>
              </div>
              <button type="button" onClick={() => { setShowCreateSprint(false); setEditingSprint(null); }} className="text-slate-400 hover:text-slate-700"><X className="h-5 w-5" /></button>
            </div>
            <div className="mt-5 space-y-4">
              <input autoFocus value={name} onChange={(event) => setName(event.target.value)} placeholder="Tên Sprint" required className="w-full rounded border border-slate-300 px-3 py-2 text-sm outline-none focus:border-blue-600" />
              <textarea value={goal} onChange={(event) => setGoal(event.target.value)} placeholder="Sprint goal (không bắt buộc)" rows={3} className="w-full resize-none rounded border border-slate-300 px-3 py-2 text-sm outline-none focus:border-blue-600" />
              <div className="grid gap-3 sm:grid-cols-2">
                <label className="text-xs font-bold text-slate-500">Start date<input type="date" value={startDate} onChange={(event) => setStartDate(event.target.value)} required className="mt-1 w-full rounded border border-slate-300 px-3 py-2 text-sm font-normal text-slate-700" /></label>
                <label className="text-xs font-bold text-slate-500">End date<input type="date" value={endDate} onChange={(event) => setEndDate(event.target.value)} required className="mt-1 w-full rounded border border-slate-300 px-3 py-2 text-sm font-normal text-slate-700" /></label>
              </div>
            </div>
            <div className="mt-6 flex justify-end gap-2">
              <button type="button" onClick={() => setShowCreateSprint(false)} className="rounded px-3 py-2 text-xs font-bold text-slate-500 hover:bg-slate-100">Hủy</button>
              <button type="submit" disabled={isBusy || !name.trim() || !startDate || !endDate} className="rounded bg-blue-600 px-4 py-2 text-xs font-bold text-white disabled:opacity-50">{editingSprint ? "Lưu thay đổi" : "Tạo Sprint"}</button>
            </div>
          </form>
        </div>
      )}
