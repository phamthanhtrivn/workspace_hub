...
export class TaskCalendarEventService implements OnApplicationBootstrap {
  ...
  async publishUpsert(
    ...
    await database.$executeRaw`
      ...
      SELECT ${crypto.randomUUID()}::uuid, 'PROJECT_TASK_CALENDAR',
        ${JSON.stringify({ taskId })}::jsonb, 'PENDING', 0, NOW(), NOW()
      WHERE NOT EXISTS (
        SELECT 1 FROM notification_outbox
        WHERE event_type = 'PROJECT_TASK_CALENDAR'
  ...
  async deliverUpsert(taskId: string): Promise<void> {
    ...
    const isRemoved = !hasSchedule || task.archived || Boolean(task.deletedAt);
    const eventType = isRemoved
